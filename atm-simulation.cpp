...
{
	char accn[max];//acount number
	char fname[20];//first name
	char lname[20];//last name
	int accp;//account password
	int balance;//the balance
	int updates;//show the account turnovers
	char last_updates[1000000];//acount updates
};
...
int main()
{
	...
	ptf = fopen("C:\\Users\\Radin\\Desktop\\New folder\\txt.txt","a+");//geting file
...
void menu()
{
    ...
    for(;;)//just let put 1 or 2 or 3
    {
    	...
    	if(ch=='1'||ch=='2'||ch=='3')
   			switch(ch)
  	    	{
    			case '1' :	employAcc();//going to employeer access 
    			break;
    			case '2' :	accessAcc();//going to customer access 
    			break;
    			case '3' :	end();
				break;
			}
...
void custom(int copy)
{
	...
	{	
		ch = getch();
		if(ch=='1'||ch=='2'||ch=='3'||ch=='4'||ch=='5'||ch=='6')//check input
		switch(ch)
	    {
	    	case '1' :	balance(copy);
	    		break;
...
void addacc()//create the acounts
{
	system("cls");
	int n ,i=10;
	printf("Enter the first name : ");//get the informathions
	scanf("%s",cost[i].fname);// first name
	fprintf(ptf,"%s ",cost[i].fname);//put informations to file
	printf("Enter the last name : ");
	scanf("%s",cost[i].lname);//last name
	fprintf(ptf,"%s ",cost[i].lname);//put informations to file
	printf("Enter the account number : ");
	scanf("%s",cost[i].accn);//acount number
	fprintf(ptf,"%s ",cost[i].accn);//put informations to file
	printf("Enter the account pasword : ");
	for(;;)//only pass more then 4 number 
	{
		scanf("%d",&cost[i].accp);//get pasword
		if(cost[i].accp < 999)
			printf("Short pasword!\nEnter another pasword : ");
		else
		{
	...
	printf("Enter the lnitial balance costomer : ");
	scanf("%d",&cost[i].balance);//put balance
	fprintf(ptf,"%d\n",cost[i].balance);//put informations to file
	system("cls");
...
void accessAcc()//customer acces
{
	...
	for(;;)//cheack that the number is excist or not
	{
		...
		if (cost[copy].balance==0&&count==1)//check the account is removed or not
		{
			...
			recentUpdate(copy);//show the last updates
		}
		if (count == 1)
		{
...
void transfer(int copy)//transfer money between accounts
{
	system("cls");
	char accn2[max],update[max];
	int i, copy2, count = 0, money,ch;
		...
		if(ch == '1'||ch == '2')
		{
			switch(ch)
			{
				case '1' : 
				{
					...
					cost[copy].updates++;//count the number of updates
				...
				case '2' : custom(copy);//go back to customer menu
					break;
			}
		}
	}
...
{
	system("cls");
	printf("%s",cost[copy].last_updates);//print the string the all updates are saved in it
	printf("\n\n\nAny another request : \n1_yes\n2_no \n");
	char ch;
	for(;;)
	{	
